...
*/
{
  ...
  int up; //(i,j,k-1)
  int west; //(i,j-1,k)
  int northup; //(i-1,j,k-1)
  int northwest; //(i-1,j-1,k)
  int westup; //(i,j-1,k-1)
  int northwestup; //(i-1,j-1,k-1)
  int south; //(i+1,j,k)
  int east; //(i,j+1,k)
  int down; //(i,j,k+1)
  int southeast; //(i+1,j+1,k)
  int southdown; // (i+1,j,k+1)
  int eastdown; // (i,j+1,k+1) /*从这里开始特征范围变小*/
  int southeastdown; // (i+1,j+1,k+1)
  int northeast; // (i-1,j+1,k)
  int northdown; // (i-1,j,k+1)
  int westsouth; // (i+1,j-1,k)
  int westdown; // (i,j-1,k+1)
  int upsouth; // (i+1,j,k-1)
  int upeast; // (i,j+1,k-1)
  int northeastdown; // (i-1,j+1,k-1)
  int northwestdown; // (i-1,j-1,k+1)
  int northupeast; // (i-1,j+1,k-1)
  int southwestup; // (i+1,j-1,k-1)
  int southwestdown; // (i+1,j-1,k+1)
  int southupeast; // (i+1,j+1,k-1)
  int NEIGHBORCOUNT = 26;
/*
  Initialization.
*/
  for ( k = 0; k < n; k++ )
    ...
    for ( j = 0; j < m; j++ )
    {
      for ( k = 0; k < n; k++ )
      {
		if (i == l - 1 || j == 0 || k == 0){
			...
			southwestup = c[i + 1 + (j - 1)*l + (k - 1)*l*m];
		}
		if (i == l - 1 || j == 0 || k == n - 1)
		{
			southwestdown = 0;
		...
		{
			southwestdown = c[i + 1 + (j - 1)*l + (k + 1)*l*m];
		}
		if (i == l - 1 || j == m - 1 || k == 0)
		{
			southupeast = 0;
		...
		{
			southupeast = c[i + 1 + (j + 1)*l + (k - 1)*l*m];
		}
		if (i == 0 || j == m - 1 || k == n - 1){
			 northeastdown = 0;
		}
		else{
			northeastdown = c[i - 1 + (j + 1)*l + (k + 1)*l*m];
		}
		if (i == 0 || j == 0 || k == n - 1)
		{
			northwestdown = 0;
			...
			northwestdown = c[i - 1 + (j - 1)*l + (k + 1)*l*m];
		}
		if (i == 0 || j == m - 1 || k == 0)
		{
			northupeast = 0;
			...
			northupeast = c[i - 1 + (j + 1)*l + (k - 1)*l*m];
		}
		if (i == l - 1 || j == 0)
		{
			westsouth = 0;
		...
		if (j == 0 || k == n - 1)
		{
			...
			westdown = c[i + (j - 1)*l + (k + 1)*l*m];
		}
		if (i == l - 1 || k == 0){
			upsouth = 0;
		}
		else{
			upsouth = c[i + 1 + j*l + (k - 1)*l*m];
		}
		if (j == m - 1 || k == 0){
			upeast = 0;
		}
		else{
			upeast = c[i + (j + 1)*l + (k - 1)*l*m];
		}
		if (i == l - 1 || j == m - 1 || k == n - 1)
		{
			southeastdown = 0;
		}
		else{
			southeastdown = c[i + 1 + (j + 1)*l + (k + 1)*l*m];
		}
		if (i == 0 || j == m - 1)
		{
			northeast = 0;
			...
			northeast = c[i - 1 + (j + 1)*l + k*l*m];
		}
		if (i == 0 || k == n - 1){
			northdown = 0;
		}
		else{
			northdown = c[i - 1 + j*l + (k + 1)*l*m];
		}
		if (i == l - 1 || j == m - 1)
		{
			southeast = 0;
		...
		if (i == 0 || j == 0 || k == 0)
		{
			...
			northwestup = c[i - 1 + (j - 1)*l + (k - 1)*l*m];
		}
		if (i == 0 || k == 0)
		{
			northup = 0;
...
/*
  New component?
*/
          if ( north == 0 && west == 0 && up == 0 && northup == 0 && northwest == 0 && westup == 0 && northwestup == 0 && south == 0 && east == 0 && down == 0 && southeast == 0 && southdown == 0 && eastdown == 0 &&southeastdown == 0 && northeast == 0 && northdown == 0 && westsouth == 0 && westdown == 0 && upsouth == 0 && upeast == 0 && northeastdown == 0 && northwestdown == 0 && northupeast == 0 && southwestup == 0 && southwestdown == 0 && southupeast == 0)
          {
            component_num = component_num + 1;
            c[i+j*l+k*l*m] = component_num;
          }
		  else{
			  //设置一个数组，分别代表north,west,up,northup,northwest,westup,northwestup的值
			  int neighborVal[26] = { north, west, up, northup, northwest, westup, northwestup, south, east, down, southeast, southdown, eastdown, southeastdown, northeast, northdown, westsouth, westdown, upsouth, upeast, northeastdown, northwestdown, northupeast, southwestup , southwestdown , southupeast };
			  int cMin = 10000;
			  int count = 0;
			  ...
			  if (count > 1)
			  {
				  ...
				  for (int i = 0; i < NEIGHBORCOUNT; i++)
				  {
					  if (neighborVal[i] != 0 && p[neighborVal[i]] < pMin)
					  {
						  pMin = p[neighborVal[i]];
					  }
				  }
				  //将p中非零的设为最小值
				  for (int i = 0; i < NEIGHBORCOUNT; i++)
...
*/
          /*else if ( north != 0 && west == 0 && up == 0 && northup == 0 && northwest == 0 && westup == 0 && northwestup == 0)
          {
            c[i+j*l+k*l*m] = north;
          }
		  else if (north == 0 && west != 0 && up == 0 && northup == 0 && northwest == 0 && westup == 0 && northwestup == 0)
		  ...
		  {
			  c[i + j*l + k*l*m] = northup;
		  }
		  else if (north == 0 && west == 0 && up == 0 && northup == 0 && northwest != 0 && westup == 0 && northwestup == 0)
		  {
			  c[i + j*l + k*l*m] = northwest;
		  }
		  else if (north == 0 && west == 0 && up == 0 && northup == 0 && northwest == 0 && westup != 0 && northwestup == 0)
		  {
			  c[i + j*l + k*l*m] = westup;
		  }
		  else if (north == 0 && west == 0 && up == 0 && northup == 0 && northwest == 0 && westup == 0 && northwestup != 0)
		  {
			  c[i + j*l + k*l*m] = northwestup;
		  }*/
/*
...
*/
		  ...
		  {
			  c[i + j*l + k*l*m] = i4_min(north, northup);
			  c1 = i4_min(p[north], p[northup]);
			  p[north] = c1;
			  p[northup] = c1;
		  }
		  else if (north == 0 && west != 0 && up == 0 && northup != 0 && northwest == 0 && westup == 0 && northwestup == 0)
		  {
			  c[i + j*l + k*l*m] = i4_min(west, northup);
			  c1 = i4_min(p[west], p[northup]);
			  p[west] = c1;
			  p[northup] = c1;
		  }
		  else if (north == 0 && west == 0 && up != 0 && northup != 0)
		  {
			  c[i + j*l + k*l*m] = i4_min(up, northup);
			  c1 = i4_min(p[up], p[northup]);
			  p[up] = c1;
			  p[northup] = c1;
		  }*/
/*
...
*/
		  ...
		  {
			  c[i + j*l + k*l*m] = i4_min(northup, i4_min(west, up));
			  c1 = i4_min(p[northup], i4_min(p[west], p[up]));
			  p[northup] = c1;
			  p[west] = c1;
			  p[up] = c1;
		  }
		  ...
		  {
			  c[i + j*l + k*l*m] = i4_min(northup, i4_min(north, up));
			  c1 = i4_min(p[northup], i4_min(p[north], p[up]));
			  p[northup] = c1;
			  p[north] = c1;
			  p[up] = c1;
		  }
		  ...
		  {
			  c[i + j*l + k*l*m] = i4_min(northup, i4_min(north, west));
			  c1 = i4_min(p[northup], i4_min(p[north], p[west]));
			  p[northup] = c1;
			  p[north] = c1;
			  p[west] = c1;
		  }
		  ...
		  {
			  c[i + j*l + k*l*m] = i4_min(northup, i4_min(north, i4_min(west,up)));
			  c1 = i4_min(p[northup], i4_min(p[north], i4_min(p[west],p[up])));
			  p[northup] = c1;
			  p[north] = c1;
			  p[west] = c1;
			  p[up] = c1;
